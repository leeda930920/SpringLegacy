...
public class TestController {
	...
	// required : false 로 설정하면 지정된 이름의 파라미터가 없는 경우
    //             null 이 주입됨  
	// @RequestParam(required=false) int num3
	//   ㄴ 일단 null 값으로 받은 다음 int type 으로 형변환하려고 함 <-- 오류발생
	// @RequestParam(required=false) String num4 <-- null 을 주입받음
	@GetMapping("/test10")
    ...
    //                                "0" 이 주입됨 
	// 이 경우에는 "0" 을 숫자 0 으로 형변환하므로 문제가 없음
	@GetMapping("/test11")
